/**
 * API Route pour ajouter des données fictives dans la base de données
 * Utile pour tester les graphiques et les fonctionnalités
 * 
 * Usage: POST /api/dev/seed-test-data
 * 
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!user) {
      return new Response(JSON.stringify({ error: 'No user found. Please create a user first.' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      })
    }
    ...
    for (const album of mockAlbums) {
      ...
      // Créer l'item sans coverUrl
      // Les covers seront ajoutées automatiquement lors de l'ajout d'albums depuis Discogs
